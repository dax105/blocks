...
public class GuiScreenMenu extends GuiScreen {
	...
	public void buttonPress(GuiObjectButton button) {
		if(button.id == 0) {
			...
			this.game.makeNewWorld(false, this.game.world.name);
		} else if(button.id == 2) {
			this.game.openGuiScreen(new GuiScreenSettings(this));
		} else if(button.id == 3) {
			this.game.world.saveAllChunks();
			this.game.world = null;
			this.game.renderEngine = new RenderEngine(Settings.getInstance().shaders.getValue());
