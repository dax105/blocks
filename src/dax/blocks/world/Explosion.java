...
public class Explosion {
	...
	public static void explode(World world, float ex, float ey, float ez) {
		...
		Game.sound.play(SoundManager.explosion, 0.8f+rand.nextFloat()*0.4f, Game.settings.sound_volume.getValue(), false);
					...
					if (blockid > 0 && blockid != Block.bedrock.getId()) {
						...
						if (distSq < radiusFuzzed*radiusFuzzed) {
							...
							if (rand.nextFloat() > 0.9f) {
								//world.spawnParticleWithRandomDirectionFast(x+rand.nextFloat(), y+rand.nextFloat(), z+rand.nextFloat(), 2, 0.5f);
							}	
						}
