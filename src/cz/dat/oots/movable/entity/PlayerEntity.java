...
import cz.dat.oots.world.BlockHitResult;
...
public class PlayerEntity extends Entity implements IOverlayRenderer {
	...
	public void updatePosition() {
		...
		boolean inWater = ((this.world.getBlockObject(blockPosX, blockPosY,
				blockPosZ) instanceof BlockFluid) || (this.world
				.getBlockObject(blockPosX, blockPosY + 1, blockPosZ) instanceof BlockFluid));
	...
	private void updateBlock() {
		...
		if(wh > 0) {
			...
			/*
			 * if(newSelectedBlock == 19) { this.inHand = new BasicItemStack(
			 * IDRegister.itemImaginaryChocolate, 1); return; }
			 */
	...
	private void updateLookingAt() {
		...
		BlockHitResult r = this.world.hitscanBlock(this.getPosXPartial(),
				this.getPosYPartial() + PlayerEntity.EYES_HEIGHT, this
						.getPosZPartial(), this.heading, this.tilt, (int) this.world
						.getGame().s().reach.getValue().floatValue());
		
		if(r.getId() <= 0) {
			this.hasSelected = false;
		} else {
			this.hasSelected = true;
			this.lookingAtX = r.getX();
			this.lookingAtY = r.getY();
			this.lookingAtZ = r.getZ();

			this.placesAtX = r.getEx();
			this.placesAtY = r.getEy();
			this.placesAtZ = r.getEz();
		}
	}
