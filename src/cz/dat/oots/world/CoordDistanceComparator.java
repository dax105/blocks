...
public class CoordDistanceComparator implements Comparator<Coord2D> {
	...
	public int compare(Coord2D arg0, Coord2D arg1) {
		float distSquared0 = Math.abs(arg0.x - this.cx)
				* Math.abs(arg0.x - this.cx) + Math.abs(arg0.y - this.cy)
				* Math.abs(arg0.y - this.cy);
		int dist0 = (int) Math.sqrt(distSquared0);

		float distSquared1 = Math.abs(arg1.x - this.cx)
				* Math.abs(arg1.x - this.cx) + Math.abs(arg1.y - this.cy)
				* Math.abs(arg1.y - cy);
