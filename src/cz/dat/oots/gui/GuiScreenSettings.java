    ...
    public GuiScreenSettings(GuiScreen parent) {
        ...
        /*this.objects
				.add(new GuiObjectRectangle(
						(parent.game.s().resolution.width() - width - overflow) / 2,
						(parent.game.s().resolution.height() - height - overflow) / 2,
						(parent.game.s().resolution.width() + width + overflow) / 2,
						(parent.game.s().resolution.height() + height + overflow) / 2,
						0xA0000000));*/
        this.objects.add(new GuiObjectTitleBar((parent.game.s().resolution
                .width() - width) / 2, (parent.game.s().resolution
                .height() - height) / 2, (parent.game.s().resolution
                .width() + width) / 2, ((parent.game.s().resolution
                .height() - height) / 2) + 30, this.f, "Options"));

        // Render distance [INTEGER SETTINGS SLIDER][ID 3]
        this.objects.add(new GuiObjectSettingsIntegerSlider(
                (parent.game.s().resolution.width() - width) / 2,
                (parent.game.s().resolution.height() - height) / 2 + 34,
                (parent.game.s().resolution.width() + width) / 2,
                ((parent.game.s().resolution.height() - height) / 2) + 58,
        ...
        this.objects.add(new GuiObjectSettingsBooleanButton(
                (parent.game.s().resolution.width() - width) / 2,
                (parent.game.s().resolution.height() - height) / 2 + 62,
                (parent.game.s().resolution.width() - width - 8) / 2 + 128,
                (parent.game.s().resolution.height() - height) / 2 + 86,
        ...
        this.objects.add(new GuiObjectSettingsFloatSlider(
                (parent.game.s().resolution.width() - width) / 2 + 128,
                (parent.game.s().resolution.height() - height) / 2 + 62,
                (parent.game.s().resolution.width() + width) / 2,
                ((parent.game.s().resolution.height() - height) / 2) + 86,
        ...
        this.objects.add(new GuiObjectSettingsBooleanButton(
                (parent.game.s().resolution.width() - width) / 2,
                (parent.game.s().resolution.height() - height) / 2 + 118,
                parent.game.s().resolution.width() / 2 - 2, ((parent.game
                .s().resolution.height() - height) / 2) + 144,
                this.f, 3, this, parent.game.s().linearFiltering));

        // Transparent leaves [SETTINGS BUTTON][ID 2]
        this.objects.add(new GuiObjectSettingsBooleanButton(
                parent.game.s().resolution.width() / 2 + 2, (parent.game
                .s().resolution.height() - height) / 2 + 118,
                (parent.game.s().resolution.width() + width) / 2,
                ((parent.game.s().resolution.height() - height) / 2) + 144,
                this.f, 2, this, parent.game.s().transparentLeaves));

        // Two pass transl. [SETTINGS BUTTON][ID 5]
        this.objects.add(new GuiObjectSettingsBooleanButton(
                (parent.game.s().resolution.width() - width) / 2,
                (parent.game.s().resolution.height() - height) / 2 + 148,
                parent.game.s().resolution.width() / 2 - 2, ((parent.game
                .s().resolution.height() - height) / 2) + 172,
                this.f, 5, this, parent.game.s().twoPassTranslucent));

        // Peaceful [SETTINGS BUTTON][ID 6]
        this.objects.add(new GuiObjectSettingsBooleanButton(
                parent.game.s().resolution.width() / 2 + 2, (parent.game
                .s().resolution.height() - height) / 2 + 148,
                (parent.game.s().resolution.width() + width) / 2,
                ((parent.game.s().resolution.height() - height) / 2) + 172,
                this.f, 6, this, parent.game.s().peacefulMode));

        // FOV [FLOAT SETTINGS SLIDER][ID 1]
        this.objects.add(new GuiObjectSettingsFloatSlider(
                (parent.game.s().resolution.width() - width) / 2,
                (parent.game.s().resolution.height() - height) / 2 + 90,
                (parent.game.s().resolution.width() + width) / 2,
                ((parent.game.s().resolution.height() - height) / 2) + 114,
                this.f, "FOV: %v", 1, this, 30, 160, parent.game.s().fov));

        // Apply/close [BUTTON][ID 1]
        this.objects.add(new GuiObjectButton((parent.game.s().resolution
                .width() - width) / 2, (parent.game.s().resolution
                .height() + height) / 2 - 24, ((parent.game.s().resolution
                .width() + width) / 2), ((parent.game.s().resolution
                .height() + height) / 2), this.f, "Close", 1, this));
    }
