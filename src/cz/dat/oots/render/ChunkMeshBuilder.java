	...
	public static ChunkMesh generateMesh(Game game, Chunk c, int cy) {
		...
		if(c != null) {
						...
						for(int y = startY; y < endY; y++) {
							...
							if(blockID > 0) {
								...
								if(block.getRenderPass() == RenderPass.OPAQUE) {
									...
									block.getRenderer().preRender(c.world,
											block, calcX, calcY, calcZ);
									block.getRenderer()
											.render(renderer, c.world, block,
													calcX, calcY, calcZ);
									block.getRenderer().postRender(c.world,
											block, calcX, calcY, calcZ);
			...
			if(translucentCount > 0) {
				...
				// All gl calls to draw the chunk (TRANSLUCENT PASS) should be
				// here
				for(int x = 0; x < 16; x++) {
					for(int z = 0; z < 16; z++) {
						for(int y = startY; y < endY; y++) {
