...
public class BlockStone extends BlockBasic {
    ...
    public BlockStone(IDRegister r) {
        ...
        rnd = new Random();
        this.setAllTextures(0).setFootStepSound(SoundManager.footstep_stone)
    ...
    public void onClick(int button, int x, int y, int z, World world) {
        if (clrScreen == null) {
            clrScreen = new ColorScreen(new Color(255, 255, 255, 255),
                    new IColorChangeCallback() {
                        @Override
                        public void onColorChanged(ColorScreen caller,
                                                   Color newColor) {
